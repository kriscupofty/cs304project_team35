...
function displayApps() {
    $.ajax({
        url: "http://localhost:1234/recruiter/apps",
        contentType: "application/json; charset=utf-8",
        type: "GET",
        success: function (res) {
            console.log(res);
            $('#apps_table').bootstrapTable({
                columns: [{field: 'aID', title: 'Appl. ID'},
                    {field: 'postingID', title: 'Posting ID'},
                    {field: 'name', title: 'Candidate Name'},
                    {field: 'resEmail', title: 'Email'},
                    {field: 'time', title: 'Date Submitted'},
                    {field: 'docPath', title: 'Document'}],
                data: res
            });
        },
        error: function (err) {
