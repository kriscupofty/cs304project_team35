...
module.exports = function (app) {
    ...
    app.get('/recruiter/apps', auth, function (req, res) {
        connection.query(`select aID, A.postingID, name, resEmail, time, docPath
                          from Application A, ResidencyCandidate C, VacancyPosting V
                          where A.resEmail = C.email and V.recruiterEmail =? and V.postingID = A.postingID`, [req.session.email],
            function (err, rows) {
                if (err) {
                    console.log(err);
                    res.status(400).send(err);
